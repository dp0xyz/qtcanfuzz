...
bool CmpDictionary::SetFromCmpData(ByteSpan cmp_data)
{
    ...
    for (size_t i = 0; i < cmp_data.size();)
    {
        ...
        if (size > DictEntry::kMaxEntrySize)
            return false;
        if (size < kMinEntrySize)
            return false;
        if (i + 2 * size + 1 > cmp_data.size())
            return false;
...
void CmpDictionary::SuggestReplacement(ByteSpan bytes, std::vector<ByteSpan> &suggestions) const
{
    if (!suggestions.capacity())
        return;
    suggestions.clear();
    if (bytes.size() < kMinEntrySize)
        return;
    ...
    for (; iter != dictionary_.end(); ++iter)
    {
        ...
        if (suggestions.size() == suggestions.capacity())
            break;
    ...
    const std::vector<ByteArray> &dict_entries)
{
    ...
    {
        if (entry.size() > DictEntry::kMaxEntrySize)
            continue;
...
{
    if (cmp_dictionary_.size() == 0)
        return false;
    if (data.size() < CmpDictionary::kMinEntrySize)
        return false;
    ...
    for (size_t i = 0; i < data.size(); i++)
    {
        ...
        if (idx + CmpDictionary::kMinEntrySize >= data.size())
            continue;
...
{
    if ((data.size() % size_alignment_) + other.size() < size_alignment_)
        return;
...
void ByteArrayMutator::MutateMany(std::vector<ByteArray> &inputs,
                                  size_t num_mutants, long unsigned int crossover_level,
...
void ByteArrayMutator::MutateOne(std::vector<ByteArray> &inputs,
                                 long unsigned int crossover_level,
                                 ByteArray &mutant)
{
    size_t num_inputs = inputs.size();
